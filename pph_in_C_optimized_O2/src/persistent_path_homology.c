...
vector *apply_border_operator_and_take_out_unmarked_points (collection_of_basis *B,
    ...
    regular_path boundary_of_path_vector1 = malloc (2 * sizeof (vertex_index));
    regular_path boundary_of_path_vector2 = malloc (2 * sizeof (vertex_index));
    regular_path boundary_of_path_vector3 = malloc (2 * sizeof (vertex_index));
    ...
    */
    ...
    while (tmp_index != NULL ) {
        ...
        if (path_dim == 2) {
            ...
            if (is_this_path_a_regular_path (boundary_of_path_vector1, path_dim - 1) == FALSE){ flag1 = 0; ++flag;}
            if (is_this_path_a_regular_path (boundary_of_path_vector2, path_dim - 1) == FALSE){ flag2 = 0; ++flag;} 
            if (is_this_path_a_regular_path (boundary_of_path_vector3, path_dim - 1) == FALSE){ flag3 = 0; ++flag;}

            for (k = 0; k < get_dimVS_of_ith_base (B, path_dim - 1) && flag < 3; ++k) {

                if (flag1 == 1
                    &&
                    are_these_regular_paths_the_same (get_path_of_base_i_index_j (B, path_dim - 1, k),
                                      boundary_of_path_vector1, path_dim - 1) == TRUE
                    &&
                    is_path_of_dimPath_p_index_j_marked (B, path_dim - 1, k) == TRUE)
                {
                    ...
                    /* u[k] = (u[k] + 1) % 2; */
                           ...
                           &&
                           are_these_regular_paths_the_same (get_path_of_base_i_index_j (B, path_dim - 1, k),
                                             boundary_of_path_vector2, path_dim - 1) == TRUE
                           &&
                           is_path_of_dimPath_p_index_j_marked (B, path_dim - 1, k) == TRUE)
                {
                    ...
                    /* u[k] = (u[k] + 1) % 2; */
                           ...
                           &&
                           are_these_regular_paths_the_same (get_path_of_base_i_index_j (B, path_dim - 1, k),
                                             boundary_of_path_vector3, path_dim - 1) == TRUE
                           &&
                           is_path_of_dimPath_p_index_j_marked (B, path_dim - 1, k) == TRUE)
                {
