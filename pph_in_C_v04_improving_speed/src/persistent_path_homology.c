...
double allow_time_vector (collection_of_basis *B,
    ...
    pthread_t thread_allow_time_vector1;
    pthread_t thread_allow_time_vector2;

    pthread_allow_time_vector_args thread_allow_time_vector1_args;
    pthread_allow_time_vector_args thread_allow_time_vector2_args;
    ...
    while (tmp != NULL) {
        ...
        if (path_dim == 2) {
            thread_allow_time_vector1_args.vertex      = temp_path[0];
            thread_allow_time_vector1_args.vertex_next = temp_path[1];
            ...
            pthread_create (&thread_allow_time_vector1,
                            NULL,
                            &pthread_allow_time_vector,
                            &thread_allow_time_vector1_args);

            pthread_create (&thread_allow_time_vector2,
                            NULL,
                            &pthread_allow_time_vector,
                            &thread_allow_time_vector2_args);

            pthread_join (thread_allow_time_vector1, NULL);
            pthread_join (thread_allow_time_vector2, NULL);
...
vector *apply_border_operator_and_take_out_unmarked_points (collection_of_basis *B,
    ...
    regular_path boundary_of_path_vector1;
    regular_path boundary_of_path_vector2;
    regular_path boundary_of_path_vector3;

    vector_index *tmp_index;

    pthread_t            thread_border1_ID;
    pthread_t            thread_border2_ID;
    pthread_t            thread_border3_ID;

    pthreadBorderOp_args thread_border1;
    pthreadBorderOp_args thread_border2;
    pthreadBorderOp_args thread_border3;

    boundary_of_path_vector  = malloc (path_dim * sizeof (vertex_index));
    boundary_of_path_vector1 = malloc (path_dim * sizeof (vertex_index));
    boundary_of_path_vector2 = malloc (path_dim * sizeof (vertex_index));
    boundary_of_path_vector3 = malloc (path_dim * sizeof (vertex_index));
    ...
    */
    ...
    while (tmp_index != NULL ) {
        ...
        if (path_dim == 2) { /* Lets use threads to help */

            boundary_of_path_vector1[0] = temp[1];
            boundary_of_path_vector1[1] = temp[2];

            boundary_of_path_vector2[0] = temp[0];
            boundary_of_path_vector2[1] = temp[2];

            boundary_of_path_vector3[0] = temp[0];
            boundary_of_path_vector3[1] = temp[1];

            thread_border1.B                       = B;
            thread_border1.path_dim                = path_dim;
            thread_border1.border_vec              = u;
            thread_border1.boundary_of_path_vector = boundary_of_path_vector1;

            thread_border2.B                       = B;
            thread_border2.path_dim                = path_dim;
            thread_border2.border_vec              = u;
            thread_border2.boundary_of_path_vector = boundary_of_path_vector1;

            thread_border3.B                       = B;
            thread_border3.path_dim                = path_dim;
            thread_border3.border_vec              = u;
            thread_border3.boundary_of_path_vector = boundary_of_path_vector3;

            pthread_create (&thread_border1_ID,
                            NULL, 
                            &pthreadBorderOp_aux,
                            &thread_border1);

            pthread_create (&thread_border2_ID,
                            NULL, 
                            &pthreadBorderOp_aux,
                            &thread_border2);

            pthread_create (&thread_border3_ID,
                            NULL, 
                            &pthreadBorderOp_aux,
                            &thread_border3);

            pthread_join (thread_border1_ID, NULL);
            pthread_join (thread_border2_ID, NULL);
            pthread_join (thread_border3_ID, NULL);

        }
        else { /* No need to use threads iin this case */
        ...
        } /* END ELSE path_dim == 0, 1 no need to use threads */
        tmp_index = tmp_index->next;
    } /*finished calculating the border*/

    free (boundary_of_path_vector);
    free (boundary_of_path_vector1);
    free (boundary_of_path_vector2);
    free (boundary_of_path_vector3);
...
/* Trying to perform the border operator faster */

void * pthreadBorderOp_aux (void * args) {

    pthreadBorderOp_args *p = (pthreadBorderOp_args*) args;

    vector              *u                      = p->border_vec;
    dim_path            path_dim                = p->path_dim;
    collection_of_basis *B                      = p->B;
    regular_path        boundary_of_path_vector = p->boundary_of_path_vector;

    unsigned int k;

    if (is_this_path_a_regular_path (boundary_of_path_vector, path_dim - 1) == FALSE) return NULL;
...
/* Trying to perform the allow_time_vector function as fast as possible */
void * pthread_allow_time_vector (void * args) {

    pthread_allow_time_vector_args *p = (pthread_allow_time_vector_args*) args;

    vertex_index        vertex                  = p->vertex;
    vertex_index        vertex_next             = p->vertex_next;
    double              *distance               = p->distance;
    graphWeightList     *W                      = p->W;
    

    *distance = network_weight (vertex, vertex_next, W);
        
    return NULL;
} 


void *pthread_entry_time (void *args) {

    pthread_entry_time_args *p = (pthread_entry_time_args*) args;

    regular_path        boundary                = p->boundary;
    double              *distance               = p->distance;
    dim_path            path_dim                = p->path_dim;
    graphWeightList     *W                      = p->W;

    if (is_this_path_a_regular_path (boundary, path_dim - 1) == FALSE) 
        *distance = 0;

    *distance = allow_time_regular_path (boundary, path_dim - 1, W);
