...
$sql_amountOfSons_root_tt =  $dbh->prepare (
                            "SELECT parent_tweet_id, COUNT(tweet_id) 
                             FROM tweet
...
$sql_amountOfSonsGenX     =  $dbh->prepare (
                             "SELECT parent_tweet_id, COUNT(tweet_id) 
                             FROM tweet
...
$sql_sonsOfGenerationX    =  $dbh->prepare (
                             "SELECT tweet_id
                             FROM tweet
                             WHERE parent_tweet_id IN (SELECT nodes.tweet_id 
                                                       FROM  nodes
...
$sql_maxSonsOfGenerationX = $dbh->prepare (
